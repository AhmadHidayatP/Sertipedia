...
class DetailVerifikasiSertifikasi extends StatefulWidget {
  const DetailVerifikasiSertifikasi({super.key, required this.title});
  final String title;

  @override
  State<DetailVerifikasiSertifikasi> createState() =>
      _DetailVerifikasiSertifikasiState();
}

class _DetailVerifikasiSertifikasiState
    extends State<DetailVerifikasiSertifikasi> {
  late String _idSertifikasi;
  ...
  void didChangeDependencies() {
    ...
    _idSertifikasi = arguments['id_sertifikasi'] ?? 0;
    ...
    fetchUserBySertifikasi();
  }

  Future<void> fetchUserBySertifikasi() async {
    try {
      final url = url_user_bySertifikasi + _idSertifikasi.toString();
      Dio dio = Dio();
  ...
  void _updateStatus(String status) async {
    try {
      final url = url_change_status_sertifikasi + _idSertifikasi.toString();
      print(url);
      Dio dio = Dio();
      Response response = await dio.put(
        url,
        data: {
          'id_sertifikasi': _idSertifikasi,
          'status': status,
        },
      );
      if (response.statusCode == 200) {
